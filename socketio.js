    ...
    socket.on("message", (data) => {
        ...
        if (data.spo2 !== 0) {
            ...
            if (healthData.objectTemp.length >= maxHealthLength) {
                ...
                db.get('SELECT id FROM Users WHERE sensorId = ?', [data.sensorId], (err, row) => {
                    if (err) {
                    ...
                    if (row) {
                        ...
                        db.run(
                            ...
                            [row[0].id, data.sensorId, average.heartRate, average.spo2, average.objectTemp],
                            (err) => {
                                if (err) {
                                    console.error('Error inserting HealthData:', err.message);
                                } else {
                                    console.log(`Inserted HealthData for userId: ${row[0].id}`);
                                }
                            }
                        )
                    }
        ...
        if (envirData.humidity.length >= maxEnvirLength) {
            ...
            db.run(
                ...
                [averageData.temperature, averageData.humidity, averageData.pm25, averageData.pm10, averageData.pm1, averageData.uv, averageData.aqi, data.sensorId],
                function (err) {
                    if (err) {
